 ...
 * 4. os eventos vão para a outbox.
 *
 ...
 *
 * Repare no que este caso de uso **não** faz: não emite documento fiscal, não
 * imprime e não gera conta a receber. Ele publica `VendaFinalizada`, e quem
 * precisa reage (ADR-0006, ADR-0016). É isso que permite o módulo fiscal ser
 * opcional sem um único `if` aqui dentro.
 */
export class FinalizarVenda {
  ...
  async executar(
    ...
    return this.unitOfWork.transacao(async (repositorios) => {
      ...
      return ok({ venda, troco: venda.troco });
    });
  }
